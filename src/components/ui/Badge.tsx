 ...
 */
export function Badge({ terrain, children }: BadgeProps) {
    const { label, classes } = terrainConfig[terrain];
    return (
        <span
            className={`inline-flex items-center border px-2 py-0.5 text-xs font-medium uppercase tracking-wide rounded-full ${classes}`}
        >
            {children ?? label}
        </span>
    );
